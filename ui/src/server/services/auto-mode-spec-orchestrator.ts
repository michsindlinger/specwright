...
export interface SpecWorktreeOps {
  ...
  mergeStoryBranchIntoSpec(projectPath: string, specBranch: string, storyBranch: string, storyWorktreePath: string, specWorktreePath: string): Promise<void>;
}
...
export class AutoModeSpecOrchestrator extends AutoModeOrchestratorBase {
  ...
  protected async onItemCompleted(itemId: string): Promise<void> {
    ...
    if (wtPath && this.gitStrategy === 'worktree' && this.specBranch && this.worktreeOps) {
      ...
      try {
        await this.worktreeOps.mergeStoryBranchIntoSpec(this.mainProjectPath, this.specBranch, branch, wtPath, this.config.projectPath);
