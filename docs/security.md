...
| 2026-09-18 | §2: Claude-Session-ID wird in der Zuordnung gespeichert und als `--resume`-Argument und für eine Existenzprüfung genutzt (UUID-geprüft, Homes sind Backend-Wissen, kein Leser); kein neuer HTTP-Endpunkt (WebSocket-Nachricht `vorhaben:session.resume` mit Validierung wie `session.assign`) (INT-2026-019) | PR folgt |
