...
| Arbeitstitel einer anhängigen Absicht-Sitzung (erste Zeile des Absichtstexts, ≤ 80 Zeichen; INT-2026-022) | intern (wie Freitext-Protokoll und erste Eingabe) | `<runtime>/vorhaben-<port>.json` (ADR-0002), im WebSocket-Snapshot `pendingIntents` | mit dem Eintrag: Claim des Ordners, Sitzungsende, Sweep nach Backend-Neustart |
...
- **Absicht-Start aus der UI (INT-2026-022):** `vorhaben:start-step` ohne Kennung startet nur in einer **neuen** Arbeitskopie — kein neuer Endpunkt, ein neuer erlaubter Wert des Sitzungsziels. Drei Schichten mit je eigenem Fehlercode, damit beim Debuggen klar ist, wer verweigert hat: der Handler prüft die **Form** der Nachricht (`sessionTarget` weglassen oder `{ kind: 'new-worktree' }` ohne Namen; sonst `INVALID_MESSAGE` = Client-Bug), der Service die **fachliche Voraussetzung** auf dem Hauptpfad der Kopie (kein Git-Repository, `cloudSessionWorktree: false` → `WORKTREE_UNAVAILABLE` = erwartete Verweigerung mit dem Text, den Michael sieht, bevor eine Sitzung existiert), der Manager ist die **letzte Sperre** für den Fall, dass sich die Welt zwischen Prüfung und Start geändert hat (`WORKTREE_NOT_A_GIT_REPO`, `WORKTREE_CREATION_DISABLED`, `WORKTREE_NAME_TAKEN` → vom Service auf `START_FAILED` „Keine Arbeitskopie möglich: …" gemappt). Validierung des Ziels wie beim Terminal (`parseSessionTarget`), Projekt muss offen sein, Antwortklasse intern; Listen und Einträge zeigen nur das Zweig-Label der Kopie, nie einen Host-Pfad (§5).
- **Abschluss aus der UI (INT-2026-024):** drei WebSocket-Nachrichten, kein HTTP-Endpunkt — `vorhaben:abschluss.vorschau` (liest die Absicht des entfernten Hauptzweigs, schreibt nichts), `vorhaben:abschluss.starten` (Bestätigung; trägt den gezeigten Basis-Commit als 40-stellige Hex-Kennung, sonst `INVALID_MESSAGE`) und `vorhaben:abschluss.zuruecknehmen`; Validierung wie `session.assign` (Projekt offen, Kennung `INT-JJJJ-NNN`), Antwortklasse intern. Das Backend schreibt nie in den Hauptcheckout: Plumbing-Commit im temporären Index, Push mit dem GitHub-Zugang des Hosts (PAT aus den Einstellungen oder SSH), Pull Request über `gh` mit `GH_TOKEN` aus demselben PAT und `GH_PROMPT_DISABLED`. Jede Meldung wird redigiert (`redactGithubTokens`) und nennt nie einen Host-Pfad — nur Zweig-, Datei- und Basisnamen (§5). Dauerhafte Eigenschaft: Abschluss-Commits laufen an den Git-Hooks des Zielprojekts vorbei (`commit-tree`); ein Projekt mit Hook-Pflichten im Commit-Text bekommt sie nicht (`architecture.md` §2).
- **Wer sieht was:**
...
| 2026-09-19 | §1: Datenobjekt „Arbeitstitel einer anhängigen Absicht-Sitzung" (intern, im Nutzerzustand); §2: Absicht-Start aus der UI nur in neuer Arbeitskopie, drei Prüfschichten mit je eigenem Code (`INVALID_MESSAGE` / `WORKTREE_UNAVAILABLE` / `START_FAILED`), kein neuer Endpunkt (§6 Zeile 1 erfüllt), Labels statt Host-Pfade (INT-2026-022) | PR offen |
| 2026-09-22 | §2: Abschluss aus der UI — drei WebSocket-Nachrichten mit Validierung (Kennung, `baseSha` 40 Hex), kein HTTP-Endpunkt (§6 Zeile 1 erfüllt); GitHub als externes System (Zugang PAT/SSH + `gh` mit `GH_TOKEN`, Ausfall = Verweigerung oder Rückbau mit Meldung; §6 Zeile „externes System" erfüllt); Meldungen redigiert, ohne Host-Pfade; Hook-Eigenschaft der Abschluss-Commits benannt (INT-2026-024) | PR offen |
