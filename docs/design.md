...
| Seite „Neue Absicht": Formular und Liste (INT-2026-022) | das Formular (Textfeld, Modellwahl, „Starten") steht immer oben; darunter, nur wenn es welche gibt, „Laufende Absicht-Sitzungen · n" — je Sitzung eine Zeile mit Punkt in der Zustandsfarbe, Sitzungsname, Zweig-Label der Arbeitskopie, Zustandstext, „Dein Text wird nach der ersten Frage übergeben" bis zur Zustellung, rechts „Im Terminal öffnen ↗" für genau diese Sitzung; die Sitzung des angedockten Terminals ist hervorgehoben; teilen sich Sitzungen eine Kopie, steht unter der letzten ein grauer Satz „Beide laufen in ‚…' — der nächste Ordner wird der älteren Sitzung ‚…' zugeordnet" (bei mehr als zwei „n Sitzungen … ältesten"). „Starten" legt immer eine neue Arbeitskopie an; geht das nicht, steht der Grund mit nächstem Schritt unter dem Knopf, der Text bleibt im Feld (Muster „Fehler nach Aktion"). Keine Karte „Absicht-Sitzung läuft" mehr | `aos-neue-absicht` (`.sitzungen`, `.eintrag.gewaehlt`, `.gruppe-hinweis`), `aos-vorhaben-view.neuSitzung` |
| Eintrag ohne Ordner in der Übersicht (INT-2026-022) | eine begonnene Absicht steht sofort in „Wartet auf dich" oder „Läuft" — dieselbe Zeile wie ein Vorhaben, nur mit der Kennzeichnung „Absicht · entsteht" statt der Kennung, dem Arbeitstitel (erste Zeile des Absichtstexts, sonst der Sitzungsname) als Titel und „Sitzungsname · Zweig-Label" statt der Phase; Einträge stehen vor den Zeilen ihrer Gruppe, neueste zuerst; Eintrag und Zeile derselben Sitzung stehen nie zusammen; Klick führt am Mac auf „Neue Absicht" mit dieser Sitzung im Terminal und in der Liste hervorgehoben (Adresse `neu/<projekt>/<sitzung>`), am Handy ins Vollbild-Terminal; die Glocke führt bei einer Absicht-Sitzung auf genau diese Sitzung | `aos-vorhaben-zeile` (`pending`, `.badge.entsteht`), `vorhaben-sort.groupRows`, `glocke-ziel` |
...
| Vorhaben abschließen (INT-2026-024) | zweitrangiger Knopf „Abschließen" in der Aktionen-Zeile (Spec, Plan, Bau, PR, unbekannt; nie in Absicht oder Umgesetzt), am Handy untereinander; Klick holt die Vorschau vom Hauptzweig und öffnet den Bestätigungsdialog nach dem Muster des Freigabe-Dialogs (Kennung · Titel, Absichtsdatei, „Auf dem Hauptzweig: Status …, Version a → b", Protokollzeile im Wortlaut, Zweig · Pull Request gegen Basis, grauer „Nicht:"-Satz; Knöpfe „Abbrechen" / „Abschließen und PR eröffnen"); während des Abschlusses gesperrt mit „Abschluss läuft …", danach Toast „Abschluss-PR #n eröffnet"; ein Fehlschlag steht als Zeile „Nicht abgeschlossen: Ursache — nächster Schritt" unter dem Knopf, bis erneut geklickt wird; mit Marke oben eine neutrale Hinweis-Zeile „Abschluss angestoßen · PR #n ↗ — nach dem Merge den Hauptcheckout aktualisieren" und statt „Abschließen" der Knopf „Abschluss zurücknehmen" (Dialog: zwei Sätze, Zweig und PR bleiben) | `aos-vorhaben-seite` `.abschliessen`, `.dialog.abschluss`, `.hinweis.abschluss`, `.abschluss-fehler`, `.zuruecknehmen` |
| Übersicht: umgesetzte Zeilen (INT-2026-024) | eine Zeile mit Marke steht unter „Umgesetzt" mit Phase „Umgesetzt" und dem Zusatz „Abschluss-PR #n" neben dem Phasen-Badge, bis die Datei im Hauptcheckout `umgesetzt` trägt (dann ohne Zusatz); der Hauptcheckout gewinnt bei `umgesetzt` vor jeder Kopie; eine Sitzung, die nur wartet (fertig, ruhig, beendet), hält eine umgesetzte Zeile nicht mehr unter „Wartet auf dich" — nur eine arbeitende oder dialogische tut das (schränkt INT-2026-016 AK-01 und INT-2026-004 FA-06 für umgesetzte Vorhaben ein) | `vorhaben-sort.groupOf`, `vorhaben-reader.mergeCandidates`/`toRow` (`phaseNote`) |
...
| 2026-09-18 | §5 Kasten „Nächster Schritt": ehrlicher Grund bei getipptem Text in der Eingabezeile (mit Zitat des Textes) statt „Sitzung arbeitet"; ein Räum-Knopf war geplant und fiel nach der Messung weg (INT-2026-021) | PR #85 |
| 2026-09-19 | §4 Muster „Seite „Neue Absicht": Formular und Liste" und „Eintrag ohne Ordner in der Übersicht" (Karte entfällt, Glocke führt auf die Sitzung); §5 Merker-Regel der Seite mit mehreren Sitzungen, Adresse nach Start bleibt (INT-2026-022) | PR offen |
| 2026-09-19 | §4 Kasten „Nächster Schritt": der Autovorschlag von Claude Code blockiert den Knopf nicht mehr — abgelehnt wird nur noch bei getipptem Text (INT-2026-023) | PR offen |
| 2026-09-22 | §4 Muster „Vorhaben abschließen" (Knopf, Bestätigungsdialog, Fehlerzeile, Hinweis mit PR-Link, „Abschluss zurücknehmen") und „Übersicht: umgesetzte Zeilen" (Zusatz „Abschluss-PR #n", Hauptcheckout gewinnt, wartende Sitzung hält nicht fest); Ist-Screenshots unter `intent/INT-2026-024-vorhaben-abschliessen/design/ist/` (INT-2026-024) | PR offen |
