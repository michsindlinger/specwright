...
| AR-08 | Die UI tippt nur in einen Zustand, den sie gerade vom Bildschirm gelesen hat, und liest nach jeder Taste nach — für Dialog-Antworten ebenso wie für Aufräum-Tasten außerhalb von Dialogen (Leeren der Eingabezeile). Die Zahl der Tasten je Vorgang ist fest begrenzt; bleibt der Bildschirm unpassend, wird abgebrochen statt weitergetippt. | Blind gesendete Tasten treffen eine Oberfläche, die sich zwischen Lesen und Tippen geändert haben kann; Esc unterbricht eine laufende Runde | Review; `dialog-driver.ts`, `plan-review-orchestrator.ts`, `vorhaben-service.ts` (`leereEingabe`) |
