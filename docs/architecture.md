...
| Web-UI Backend | Projekte, Sessions, Cloud-Terminal (tmux; startet auch fremde Agenten-CLIs wie Codex nativ aus der Modell-Config — ohne Hooks, Status und Prüfer-Einsatz; Regel `shared/provider-cli.ts`, INT-2026-012), Vorhaben-Sicht und Review-Kanal (Antworten als Bracketed Paste in die wartende PTY, Bestätigung über den `UserPromptSubmit`-Hook; Freitext nur nach Bildschirmprüfung auf Dialog-Cues unter dem Maschinen-Lock `withMachineWrite`, INT-2026-007), Hook-Route (Status, Blockart, Kontext je Sitzung — kein Transkript-Leser, INT-2026-011; dazu je Sitzung die Marke „fertig, unbeantwortet" für die Glocke, in der Registry bis 24 h, INT-2026-016) als erste Statusquelle **plus Bildschirm-Probe bei Stille als zweite**: eine `working`-Claude-Sitzung, die 1,5 s nichts ausgibt, wird einmal per tmux gelesen; ein Dialog-Cue setzt `blocked` mit Blockart und Herkunft `probe`, ein verschwundener Cue hebt nur einen Probe-Block wieder auf, ein Hook-Ereignis gewinnt immer (INT-2026-016, kein ADR-Wechsel — Bildschirm, nicht Transkript, wie die Sendeprüfung INT-2026-007); **Wiederaufnahme-Regel** (INT-2026-019): das Öffnen der Vorhaben-Seite (`vorhaben:session.resume`) lässt den Vorhaben-Dienst entscheiden, ob die Sitzung der Zeile verloren ist — Zuordnung nicht beendet, Sitzung dem Manager unbekannt —, und startet sie dann über den normalen Startpfad mit `claude --resume <Gesprächskennung>` ohne Eingabe in derselben Arbeitskopie (single-flight je Zeile, Vorprüfung: Transkript-Datei vorhanden, Arbeitskopie vorhanden, Boot-Restore abgeschlossen); **Schutzregel des Aufräumers**: der Sitzungsverwalter bekommt beim Bau zwei Guards (`keepWorktree`, `sessionEnded`) und lässt beim Boot-Reap und im Shutdown saubere Sitzungs-Arbeitskopien stehen, die ein offenes Vorhaben belegen; eine Exit-Datei des Run-Scripts macht ein reguläres Ende während des Stillstands als „beendet" kenntlich (keine Wiederaufnahme); **Nächster Schritt in der Sitzung** (INT-2026-018): die Zeile trägt die Freigabe-Regel des Knopfs (`deriveNextStepSperre` — arbeitet, Dialog, Zustand unbekannt, erste Eingabe unterwegs, Freigabe offen, gleiche oder spätere Phase) und die wiederverwendbare Sitzung; der Klick schreibt `/clear` und den Phasen-Befehl als zwei Bracketed Pastes unter **einem** `withMachineWrite` und wartet dazwischen auf die neue Gesprächskennung aus dem SessionStart-Hook (Zeitüberschreitung = Abbruch mit Meldung, der Befehl geht nie in ein volles Gespräch; vor jedem Paste muss der Bildschirm die leere Eingabezeile ohne Spinner zeigen, `promptZustand` — maßgeblich ist die **unterste** Prompt-Zeile, und steht dort noch getippter Text, ist das ein eigener Grund `eingabe_nicht_leer` mit dem Fehlercode `PROMPT_NOT_EMPTY`, kein „arbeitet"; die UI räumt die Zeile **nicht** selbst — gemessen ist das über den PTY-Pfad nicht verlässlich, INT-2026-021. **Gemalter Text allein entscheidet das nicht** (INT-2026-023): Claude Code zeichnet nach jeder Runde den letzten Befehl als Vorschlag in die leere Eingabezeile. Meldet der Bildschirmtext `eingabe_nicht_leer`, holt die Prüfung deshalb eine zweite, jüngere tmux-Aufnahme — ungefalteter Pane **und** `cursor_x`/`cursor_y` aus **einem** Aufruf (`captureCursorProbe`) — und lässt den Klick nur durch, wenn der Cursor in einer Zeile mit dem Eingabezeichen steht, links von ihm nur Leerraum liegt und dieselbe Aufnahme weder Spinner noch Dialog-Cue trägt (`eingabeLeerLautCursor`); ohne Aufnahme bleibt es bei der Ablehnung, und der zitierte Text entsteht erst danach), anderes Modell oder anderes Ziel → neue Sitzung, die alte wird wie per ✕ geschlossen (`closeSession(id, { closedBy: 'user' })`); **Absicht-Start immer in neuer Arbeitskopie** (INT-2026-022): `vorhaben:start-step` ohne Kennung nimmt nur kein Ziel oder `new-worktree` ohne Namen — drei Schichten mit je eigenem Code: der Handler prüft die Form (`INVALID_MESSAGE`), der Service die fachliche Voraussetzung auf dem Hauptpfad der Kopie (kein Git-Repository, Isolation abgeschaltet → `WORKTREE_UNAVAILABLE` mit dem Text, den Michael sieht), der Manager ist die letzte Sperre (`WORKTREE_NOT_A_GIT_REPO` für explizite Ziele, nur Alt-Aufrufer ohne Ziel laufen degradiert im Projektordner) und jeder Manager-Fehler wird zu `START_FAILED` „Keine Arbeitskopie möglich: …"; nie ein stiller Start im Hauptcheckout. Der Scan liest die Worktree-Liste am 5-s-Cache vorbei, wenn eine anhängige Absicht-Sitzung in einer Kopie läuft, die der Cache nicht kennt (sonst bliebe ihr Ordner unbeobachtet); tote anhängige Einträge räumt der Scan nur bei `closed` oder bei unbekannter Sitzung **und** Boot-Restore `complete` (nie während `pending`/`timeout` — der Absichtstext einer spät wiederhergestellten Sitzung geht nicht verloren), WebSocket | Express, TypeScript, Claude Code SDK, node-pty | `ui/src/server/` | Michael |
| Web-UI Frontend | Oberfläche als Web Components: Rahmen `aos-kopfzeile` mit `aos-glocke` (INT-2026-010; Einträge seit INT-2026-016 aus dem Backend-Stand der Sitzungen — Dialog oder Marke —, Vorhaben-Zeilen nur als Beschriftung, keine Browser-Merkliste), drei Routen `vorhaben`, `neu`, `projekt` (Aliase der alten Adressen in `route.types.ts`; `neu/<projekt>/<sitzung>` trägt seit INT-2026-022 die gewählte Absicht-Sitzung als zweites Segment — Klick aus der Übersicht, Glocke, Neuladen; nach „Starten" bleibt die Adresse stehen und die View merkt sich die gestartete Sitzung flüchtig); auf `vorhaben` und `neu` ist die Cloud-Terminal-Sidebar angedockt (`docked`: halbe Breite unter der Kopfzeile ab 1024 px, dieselbe Instanz wie schwebend) und die Kennungen des Dokuments sind im Terminal Verweise (`kennungen.service`, `KennungLinkProvider`, INT-2026-011); Git-Zustand prozesslang in `services/git-state.service.ts`, Git-Leiste und Einstellungen als Abschnitte der Projekt-Seite | Lit, Vite, TypeScript strict | `ui/frontend/src/` (`aos-*`) | Michael |
...
| AR-08 | Die UI tippt nur in einen Zustand, den sie gerade vom Bildschirm gelesen hat, und liest nach jeder Taste nach; die Zahl der Tasten je Vorgang ist fest begrenzt, und bleibt der Bildschirm unpassend, wird abgebrochen statt weitergetippt. Ein Bildschirm, den die UI **nicht** selbst ausgelöst hat, gilt nicht als Bestätigung: eine Rückleseprobe kann ein altes Bild zeigen (INT-2026-021). | Blind gesendete Tasten treffen eine Oberfläche, die sich zwischen Lesen und Tippen geändert haben kann; Esc unterbricht eine laufende Runde und öffnet auf leerer Eingabezeile den Rewind-Wähler | Review; `dialog-driver.ts`, `plan-review-orchestrator.ts` |
...
| 2026-09-18 | §2 Backend: die strenge Bildschirmprüfung nennt vier Zustände statt zwei (`promptZustand`); getippter Text in der Eingabezeile wird eigener Grund mit eigenem Fehlercode statt „arbeitet", maßgeblich ist die unterste Prompt-Zeile; neue Regel **AR-08** (Tasten nur in einen gelesenen Zustand, Nachlesen nach jeder Taste, feste Obergrenze, Rückleseprobe ist kein Beweis) — sonst keine AR-Änderung, kein neues Datenobjekt (INT-2026-021) | PR #85 |
| 2026-09-19 | §2 Backend: Absicht-Start immer in neuer Arbeitskopie in drei Schichten (Handler/Service/Manager mit je eigenem Code), Scan-Selbstprüfung gegen den Worktree-Cache, Sweep toter anhängiger Einträge gebunden an den Boot-Restore; §2 Frontend: Route `neu/<projekt>/<sitzung>`; §3 Nutzerzustand: Arbeitstitel (≤ 80 Zeichen) als einzige, benannte Ausnahme der Flag-Regel, Einträge mit Projektname, Zustand und Zweig-Label — keine AR-Änderung, AR-03 (bestehender Hauptrepo-Lock), AR-05 (Auswahl in Adresse und Backend-Stand) eingehalten, Erweiterung von ADR-0002 um ein optionales Feld wie INT-2026-019 (INT-2026-022) | PR offen |
| 2026-09-19 | §2 Backend: die strenge Bildschirmprüfung entscheidet `eingabe_nicht_leer` nicht mehr am gemalten Text allein — eine zweite tmux-Aufnahme (ungefalteter Pane plus Cursorposition, ein Aufruf) gibt den Ausschlag, samt Nachprüfung auf Spinner und Dialog auf derselben Aufnahme; ohne Aufnahme unverändert fail closed. Keine AR-Änderung (AR-08 wird eingehalten), kein neues Datenobjekt, kein neuer Fehlercode (INT-2026-023) | PR offen |
