...
#
# Usage:
#   ./auto-execute.sh [spec-name] [-v|--verbose] [-P|--provider NAME] [-a|--anthropic] [-g|--glm] [-G|--gemini]
#
# Example:
...
#   ./auto-execute.sh --gemini 2026-01-13-feature     # Use Gemini
#
...
run_phase() {
    ...
    while [[ $retry -lt $MAX_RETRIES ]]; do
        ...
        if [[ "$PROVIDER" == "anthropic" ]]; then
            # Note: We rely on the CLI default or config here.
            # If vous need --dangerously-skip-permissions, add it to the MODEL string or PROVIDER command.
            cmd="claude-anthropic-simple -p \"/agent-os:execute-tasks $spec\" --model $MODEL"
        elif [[ "$PROVIDER" == "gemini" ]]; then
            cmd="claude-gemini -p \"/agent-os:execute-tasks $spec\" --model $MODEL"
        else
            # For backward compatibility, we keep it here but allow skipping via PROVIDER override
        ...
        if [[ "$PROVIDER" == "anthropic" ]]; then
            ...
            claude-gemini -p "/agent-os:execute-tasks $spec" \
                --model "$MODEL" \
                2>&1 | tee "$log_file"
        else
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -P|--provider)
                PROVIDER="$2"
                if [[ "$PROVIDER" != "anthropic" && "$PROVIDER" != "glm" && "$PROVIDER" != "gemini" ]]; then
                    echo "Error: Provider must be 'anthropic', 'glm' or 'gemini'"
                    exit 1
                fi
                shift 2
                ;;
