...
| 2026-09-17 | `origin/main` in den Branch gemerged (PR #61/#65/#66 „Terminal statt Gespräch", #68, #69): der Gesprächs-Verlauf (`gespraech-service`, `aos-gespraech`) existiert nicht mehr — AK-03 „Gespräch zeigt den Verlauf" ist gegenstandslos, die E2E-Messung dazu stammt vom Stand vor dem Merge (Protokoll bleibt als Beleg). Konflikte in `architecture.md` §2/Protokoll und den Imports von `cloud-terminal-manager.ts`; Wortlaut „Gespräch" aus `provider-cli.ts`, `model-config.json`-Beschreibung und Kommentar entfernt; `getAllProviders`-Import in `websocket.ts` ohne Aufrufer gestrichen. `verify: OK` nach dem Merge | Main lief während des Baus weiter | §2 (Gespräch-Zeile), §4 Nr. 1/2/3/11, §8 AK-03 |
