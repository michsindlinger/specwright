...
| Merge nach `main` → Auto-Deploy der UI auf dem Cloud-Host (Weg: `.github/workflows/verify.yml` als Tor, Deploy-Timer außerhalb des Repos, `docs/architecture.md` §5); Hook `production-gate` nicht betroffen (kein `deploy`+`prod`-Befehl) | Michael | Merge | [ ] |
