...
- [x] `bash scripts/verify.sh` → nach dem Merge im zweiten Lauf **`verify: OK`**; der erste Lauf war rot durch eine wechselnde fremde Testdatei (§14, Stufe 5 lokal flaky). **PR-Check `verify` grün** (Lauf 35263712088) — CI ist die Wahrheit.
...
| 2026-09-18 | Nach dem Merge meldete Stufe 5 erneut eine „neue rote Testdatei", diesmal `tests/unit/cloud-terminal-paste-image.test.ts` (aus INT-2026-020, von `main`); zweiter Lauf desselben Befehls ohne Änderung: `verify: OK` | Beide Dateien sind einzeln grün (`paste-image` auch im `main`-Checkout, 6/6), rot nur im vollen Parallellauf — Interferenz zwischen Testdateien, kein Befund dieses Vorhabens. Zusammen mit dem gestrigen Befund (`terminal-io.test.ts`, heute grün) heißt das: die lokale Stufe 5 wechselt die rote Datei von Lauf zu Lauf. Bezugsliste unangetastet | §13 (Verify-Punkt erfüllt: lokal `verify: OK` im zweiten Lauf, PR-Check grün); Board-Karte um „Stufe 5 lokal flaky" erweitern |
| 2026-09-18 | `main` in den Branch gemergt (35 Commits: INT-2026-018, -019, -020), ein Konflikt in `docs/design.md` (Änderungsprotokoll, beide Seiten hängten Zeilen an) | Der PR lag über Nacht offen und wurde CONFLICTING; die Kennung INT-2026-017 wurde in einer anderen Sitzung zweitvergeben und dort auf INT-2026-020 umbenannt (kein Ordner-Konflikt). Gelöst durch Übernahme aller Protokollzeilen in Datumsreihenfolge; die 017-Zeile trägt jetzt PR #76. `aos-vorhaben-seite.ts` und der Chips-Test mergen automatisch — Test danach 15/15 grün (main hat einen Fall ergänzt) | §6 Schritt 8, kein Code dieses Vorhabens |
